...
def tokenize(fname):
    ...
    while i < n:
        ...
        if header_re.match(line):
            ...
            i += 1
            # Accumulate a balanced [...] scope block across however many lines it takes.
            buf = ''
            depth = 0
            saw_open = False
            while i < n:
                ...
                if saw_open and depth == 0:
                    break
            try:
                scopes = ast.literal_eval(buf.strip())
            except Exception:
                continue
            if not isinstance(scopes, list) or key == '':
                continue
            pairs.append((key, scopes))
        else:
            i += 1
